...
var Game = /** @class */ (function () {
    ...
    Game.prototype.placeBlock = function () {
        ...
        // Notify MultiSynq about block placement (using a custom event)
        var blockPlacedEvent = new CustomEvent('blockPlaced', { 
            detail: { 
                ...
                dimensions: {
                    width: currentBlock.dimension.width,
                    height: currentBlock.dimension.height,
                    depth: currentBlock.dimension.depth
                },
                bonus: newBlocks.bonus || false
            } 
        });
        document.dispatchEvent(blockPlacedEvent);
    ...
    Game.prototype.endGame = function () {
        ...
        // Play game over sound effect (if we have music enabled)
        const backgroundMusic = document.getElementById('background-music');
        if (backgroundMusic && localStorage.getItem('towerBlocks_musicEnabled') !== 'false' && !backgroundMusic.paused) {
        ...
        const multisynqPanel = document.getElementById('multisynq-panel');
        if (multisynqPanel) {
            const highScoresList = document.getElementById('highScoresList');
            if (highScoresList && highScoresList.children.length > 0) {
                const topScore = parseInt(highScoresList.querySelector('li')?.textContent?.match(/\d+$/)?.[0] || '0');
...
// Make Game constructor available globally
window.GameConstructor = Game;
